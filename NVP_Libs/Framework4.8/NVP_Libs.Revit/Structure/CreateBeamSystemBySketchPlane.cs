...
namespace NVP_Libs.Revit.Structure
{
    ...
    internal class CreateBeamSystemBySketchPlane : INode
    {
        public NodeResult Execute(INVPData context, List<NodeResult> inputs)
        {
            ...
            using (Transaction transaction = new Transaction(doc, "Создание балочной системы по эскизу"))
            {
                transaction.Start();
                BeamSystem beamSystem = BeamSystem.Create(doc, profile, sketchPlane, direction, is3D);
                transaction.Commit();
